...
profit_taking = [0.2,0.5,0.7] #if (unrealized pnl/days + another 1 day of carry) / collateral > 70%, then it means I expect to make 30%/50%/70% on daily basis, lock in the profit 
buffer = [0.0005,0.001,0.002] #annualized expected return to be 18.25%, 36.5%, 73%
withdraw_fee = [0]
...
for n in range(0,len(combo_mix)):
    profit_taking = combo_mix.iloc[n,0]
    buffer = combo_mix.iloc[n,1]
    withdraw_fee = combo_mix.iloc[n,2]
    method = combo_mix.iloc[n,3]
    ...
    #the script for leveragedCarry
    
    def leveragedCarryVault(pair,i):#self = vault[pair] 
        maxNotional=0
        notional=0
        vault[pair]['apy_history'][i] = apy_history[pair][i]
        vault[pair]['instant_carry%'][i]= (apy_history[pair][i]-amm[pair]['Quote_initial'][i])/30*365 #instant carry if long or short at the current market level 
        ...
        #notional takes the min (maxNotional, 10k) with increment of 1k
        ...
        for notional in range(1000,int(maxNotional),1000):
            ...
            if vault[pair]['expected_return'][i]>0:
                ...
                estimated_slippage = ((amm[pair]['Staked_amount_Long'][i]+notional)/(amm[pair]['Staked_amount_Short'][i])/amm[pair]['Ratio'][i]-1)*notional*2
            else:
                estimated_slippage = (amm[pair]['Ratio'][i]/(amm[pair]['Staked_amount_Long'][i]/(amm[pair]['Staked_amount_Short'][i]+notional))-1)*notional*2
            #expected USDC income if holding the position for a day (since we rebalance each day )
            expected_return = vault[pair]['side'][i]*notional*vault[pair]['expected_return'][i]/100 #monthly return based on notional
            ...
            if (expected_return - expected_cost)>max:
                ...
                pass 
        if i == 0:       
            if vault[pair]['expected_return'][i]/100/365>buffer: 
                vault[pair]['side'][i]=1
                amm[pair]['Staked_amount_Long'][i]+= vault[pair]['notional'][i]
                amm[pair]['net_exposure'][i] -= vault[pair]['notional'][i]
            elif vault[pair]['expected_return'][i]/100/365<-1*buffer: 
                vault[pair]['side'][i]=-1
                amm[pair]['Staked_amount_Short'][i]+= vault[pair]['notional'][i]
                amm[pair]['net_exposure'][i] += vault[pair]['notional'][i]
            else:
            ...
            vault[pair]['cumulative_Trading_vol'][i] = vault[pair]['notional'][i] #because rewards only give out on daily basis 
            amm[pair]['cumulative_Trading_Vol_rebased_agg'][i] += vault[pair]['notional'][i]

            #update the trading income into AMM 
            vault[pair]['trading_fee_in'][i]=vault[pair]['notional'][i]*vault[pair]['avg_price'][i]*0.01*0.01
            vault_summary['Cash_reserves'][i] -= vault[pair]['trading_fee_in'][i]
            vault_summary['USDC_Balance'][i] -=vault[pair]['trading_fee_in'][i]
            ...
            #there is no realized pnl in day 1 
          ...
          #no existing position 
            ...
            #open new (same as day 0)
                if vault[pair]['expected_return'][i]/365/100>buffer: 
                    vault[pair]['side'][i]=1
                    amm[pair]['Staked_amount_Long'][i]+= vault[pair]['notional'][i]
                    amm[pair]['net_exposure'][i] -= vault[pair]['notional'][i]
                elif vault[pair]['expected_return'][i]/365/100<-1*buffer: 
                    vault[pair]['side'][i]=-1
                    amm[pair]['Staked_amount_Short'][i]+= vault[pair]['notional'][i]
                    amm[pair]['net_exposure'][i] += vault[pair]['notional'][i]
                else:
                ...
                vault[pair]['cumulative_Trading_vol'][i] = vault[pair]['notional'][i] #because rewards only give out on daily basis 
                amm[pair]['cumulative_Trading_Vol_rebased_agg'][i] += vault[pair]['notional'][i]
                #because it is new position being initiated just now, so no unrealized pnl at all 
                # minus trading fee and collateral when open new positions 
                vault[pair]['trading_fee_in'][i]=vault[pair]['notional'][i]*vault[pair]['avg_price'][i]*0.01*0.01
                vault_summary['Cash_reserves'][i] -= vault[pair]['trading_fee_in'][i]
                vault_summary['USDC_Balance'][i] -=vault[pair]['trading_fee_in'][i]
                vault_summary['Cash_reserves'][i] -= vault[pair]['collateral'][i]
            ...
            #there is open position 
                ...
                #not day 1, check the following         
                #update the Unrealized PnL of the vault and inheret values from last day first 
                vault[pair]['position'][i] = vault[pair]['position'][i-1]
                vault[pair]['Rebalance_count'][i] = vault[pair]['Rebalance_count'][i-1]
                vault[pair]['collateral'][i] = vault[pair]['collateral'][i-1]
                vault[pair]['avg_price'][i] = vault[pair]['avg_price'][i-1]
                vault[pair]['side'][i] = vault[pair]['side'][i-1]
                vault[pair]['Unrealized_PnL'][i] = calcUnrealized_perp(vault[pair],i)
                #no need to inheret the cumulative trading volume since it is reset every 24 hours for the vault and traders 
                vault[pair]['STRP_reward'][i] = vault[pair]['STRP_reward'][i-1]
                #add collateral to avoid being liquidated with cash reserves, up to 10% (original leverage level)
                if (vault[pair]['collateral'][i]+vault[pair]['Unrealized_PnL'][i])/vault[pair]['position'][i]<0.1:
                    ...
                    if vault_summary['Cash_reserves'][i] <0:
                        ...
                        """at this moment, there is nothing we can do if cash reserves are used out to add new collateral, we cannot choose to close position if we don't top up the collatearl to bring back the margin level above 5%"""
                #close to cut loss (if my unrealized loss already overpasses my 30days of expected returns) to avoid being liquidated OR
                #close to cut loff (if my unrealized pnl + 1 day expected return of current position)<0
                #close to take profit if my daily return (over period of holding) > 70%  
                if (vault[pair]['Unrealized_PnL'][i]+vault[pair]['side'][i-1]*vault[pair]['expected_return'][i]/100/365*30*vault[pair]['position'][i]) <0 or\
                    (vault[pair]['Unrealized_PnL'][i]/vault[pair]['days_elapsed'][i]+vault[pair]['expected_return'][i]/100/365*vault[pair]['position'][i])/vault[pair]['collateral'][i]>profit_taking:  
                    #make sure I would not close the full position that slippage would be too large
                    print('rebal: point a for '+str(pair))
                    ...
                    if vault[pair]['side'][i-1]==-1:#if trader is short
                    ...
                    #calculate Realized PnL on the closed position 
                    vault[pair]['Realized_PnL'][i] = calcRealized_perp(vault[pair],i)    
                    ...
                    vault[pair]['trading_fee_out'][i]=vault[pair]['exit_level'][i]*0.01*vault[pair]['notional'][i]*0.01
                    #vault[pair]['actual_received'][i]=vault[pair]['Realized_PnL'][i]              
                    ...
                    vault_summary['Cash_reserves'][i] -= vault[pair]['trading_fee_out'][i] #charge the trading fee when partially closing the position 
                    vault_summary['USDC_Balance'][i] -=vault[pair]['trading_fee_out'][i]
                    vault_summary['Cash_reserves'][i] += vault[pair]['Realized_PnL'][i] #charge or receive the realized pnl from partially closing the position 
                    vault_summary['USDC_Balance'][i] += vault[pair]['Realized_PnL'][i]
                ...
                #otherwise hold if the sign of the trade changes 
                    ...
                    print('rebal: point b for '+str(pair))
                    #other position information are already inhereted 
                    #note we "estimated the optimal size to trade before any date, so we need to change it back 
                    vault[pair]['notional'][i] = 0 #no more new trade today 
                    vault[pair]['position'][i] = vault[pair]['position'][i-1]
                    vault[pair]['side'][i] = vault[pair]['side'][i-1]
                    vault[pair]['collateral'][i] = vault[pair]['collateral'][i-1] #nothing changed on collateral
                    vault[pair]['trading_fee_in'][i] = 0 #no more new trading 
                    vault[pair]['days_elapsed'][i] = vault[pair]['days_elapsed'][i-1]
                    vault[pair]['Unrealized_PnL'][i] = calcUnrealized_perp(vault[pair],i)
                    #nothing changesa AMM and vault afterwards 
                #add more if sign is the same and still > buffer (same as open new position)
                elif (vault[pair]['Unrealized_PnL'][i]+vault[pair]['side'][i-1]*vault[pair]['expected_return'][i]/100/365*30*vault[pair]['position'][i])>0 and np.sign(vault[pair]['expected_return'][i]) == np.sign(vault[pair]['expected_return'][i-1]):
                    #notional is already decided at the for_loop for pairs before
                    if vault[pair]['expected_return'][i]/365/100>buffer:
                        print('rebal: point c for '+str(pair))
                        vault[pair]['side'][i]=1
                        amm[pair]['Staked_amount_Long'][i]+= vault[pair]['notional'][i]
                        amm[pair]['net_exposure'][i] -= vault[pair]['notional'][i]
                    elif vault[pair]['expected_return'][i]/365/100<-1*buffer: 
                        print('rebal: point d for '+str(pair))
                        vault[pair]['side'][i]=-1
                        amm[pair]['Staked_amount_Short'][i]+= vault[pair]['notional'][i]
                        amm[pair]['net_exposure'][i] += vault[pair]['notional'][i]
                    else: #no need to add more positions if < buffer (which means carry is not that attractive right now )
                        print('rebal: point e for '+str(pair))
                        vault[pair]['side'][i]=0
                        vault[pair]['side'][i]=0 
                        vault[pair]['notional'][i]=0
                        vault[pair]['collateral'][i]=0
                    ...
                    vault[pair]['collateral'][i]+= vault[pair]['notional'][i]/10
                    vault[pair]['trading_fee_in'][i] = amm[pair]['Quote_initial'][i]*0.01*0.01*vault[pair]['notional'][i]
                    vault[pair]['Unrealized_PnL'][i] = calcUnrealized_perp(vault[pair],i)
                    vault[pair]['days_elapsed'][i] = vault[pair]['days_elapsed'][i-1]+1
                    #update the cash reserves
                    vault_summary['Cash_reserves'][i]-=vault[pair]['notional'][i]/10 #collateral used 
                    vault_summary['USDC_Balance'][i]-=vault[pair]['notional'][i]/10 #collateral used 
                    price_before = (apy_history[pair][0]*initial_haircut)*amm[pair]['Ratio'][i]
                    amm[pair]['slippage'][i]=np.nanmean([amm[pair]['slippage'][i],abs(price_before - amm[pair]['Quote_initial'][i])/price_before])
                    #update the trading income into AMM 
                    vault[pair]['trading_fee_in'][i]=vault[pair]['notional'][i]*amm[pair]['Quote_initial'][i]*0.01*0.01
                    """updatee revenue, USDC_balance and staking amount of AMM, TL and TS for the next trader"""
                    #AMM only gets 75% of trading fee_in
                    amm[pair]['revenue'][i]+=vault[pair]['trading_fee_in'][i]*0.75
                    amm[pair]['USDC_Balance'][i]+=vault[pair]['trading_fee_in'][i]*0.75
                    ...
                    amm[pair]['Ratio'][i] = amm[pair]['Staked_amount_Long'][i]/amm[pair]['Staked_amount_Short'][i]
                    amm[pair]['Quote_initial'][i]=(apy_history[pair][0]*initial_haircut)*amm[pair]['Ratio'][i]
                    #update vault volume from vault daily rebalance 
                    vault[pair]['cumulative_Trading_vol'][i] += vault[pair]['notional'][i] #only accumulate every 1 day, no need to add from prev days
                    vault[pair]['Rebalance_count'][i]+=1
                    amm[pair]['cumulative_Trading_Vol_rebased_agg'][i] +=vault[pair]['cumulative_Trading_vol'][i]
                    #update vault_summary 
                    vault_summary['Cash_reserves'][i] -= vault[pair]['trading_fee_in'][i] #charge the trading fee when partially closing the position 
                    vault_summary['USDC_Balance'][i] -=vault[pair]['trading_fee_in'][i]
                    #Insurance pool gets 5% of trading fee_in and 20% goes to governence 
                    insurance['trading_fee'][i]+=vault[pair]['trading_fee_in'][i]*0.25
                    amm[pair]['apy_history'][i]=apy_history[pair][i]        
    ...
    def deposit(i,amount,method):
        #use lpPrice to decide the LP amount to be received when minting based on USDC amount deposited
        if i>0:
    ...
    for pair in currencies:
        ...
        vault[pair] = pd.DataFrame(0.0,columns=['instant_carry%','trailing_carry%','expected_return','estimated_slippage','expected_cost',\
                                                'expected_net_return','realized_trading_PnL','realized_funding_PnL','Realized_PnL',\
    ...
    for pair in currencies:            
        ...
        vault[pair]['STRP_reward'][i] = amm[pair]['trading_share'][i]*3651*vault[pair]['cumulative_Trading_vol'][i]/amm[pair]['cumulative_Trading_Vol_rebased_agg'][i]
        
    for j in range(0,50):
        pair_staker = stakers[j]['currency'][i]
        pair = traders[j]['currency'][i]
        stakers[j]['reward_staker'][i] = amm[pair_staker]['staking_share'][i]*3651*stakers[j]['Share_Percentage'][i]
        traders[j]['reward_trader'][i]+=amm[pair]['trading_share'][i]*3651*traders[j]['cumulative_Trading_Vol_rebased_'+pair][i]/amm[pair]['cumulative_Trading_Vol_rebased_agg'][i]
    ...
    for i in range(1,len(apy_history.index)):
        ...
        for pairs in currencies:            
            ...
            #give rewards to vault (the only trader)
            vault[pairs]['STRP_reward'][i] += amm[pairs]['trading_share'][i]*3651*vault[pairs]['cumulative_Trading_vol'][i]/amm[pairs]['cumulative_Trading_Vol_rebased_agg'][i]
        ...
        for j in range(0,50):
            if stakers[j]['currency'][i]!=str(0.0): #if currently unstaked, then staker will get nothing on this day 
                ...
                for pairs in currencies:    
                    traders[j]['reward_trader'][i]+=amm[pairs]['trading_share'][i]*3651*traders[j]['cumulative_Trading_Vol_rebased_'+pairs][i]/amm[pairs]['cumulative_Trading_Vol_rebased_agg'][i]
                traders[j]['reward_staker'][i] = stakers[j]['reward_staker'][i]
                traders[j]['total_reward'][i]=traders[j]['reward_trader'][i]+traders[j]['reward_staker'][i]
                stakers[j]['Undistributed_profit'][i]=amm[pair_staker]['Undistributed_profit_growth'][i]*stakers[j]['Share_Percentage'][i]
        ...
        for pair in currencies:    
            vault[pair]['STRP_reward'][i] = (amm[pair]['trading_share'][i]*3651*vault[pair]['cumulative_Trading_vol'][i]/amm[pair]['cumulative_Trading_Vol_rebased_agg'][i])
            vault_summary['STRP_reward'][i] += (amm[pair]['trading_share'][i]*3651*vault[pair]['cumulative_Trading_vol'][i]/amm[pair]['cumulative_Trading_Vol_rebased_agg'][i])
    ...
    for pair in currencies:
        ...
        collateral_summary['staking_APY']=3615*collateral_summary['STRP_Price']/collateral_summary['Staked_amount']*365*100
        collateral_summary['trading_APY']=3615*collateral_summary['STRP_Price']/(amm['aave']['cumulative_Trading_Vol_rebased_agg']+\
    ...
    parent_dir = "C:/Users/CZ/Downloads/Strips.finance/Trading Strategies/Leveraged Carry/dataset3"   
    amm_performance.to_excel(parent_dir+'amm_perforamnce.xlsx')
    
    """for each scenario"""
    directory =str(profit_taking)+"_"+str(buffer)+"_"+str(withdraw_fee)+"_"+str(method)
